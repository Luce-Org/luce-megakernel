...
// ─── MoE harness (Qwen3.5/3.6-35B-A3B with expert swapping) ──────
// Called when the loaded target model has w.is_moe == true.
// Implements chain-verify spec-decode with ExpertCache for expert weight swapping.
    ...
    bool ddtree_chain_seed = true)
{
    ...
    if (!ecache.init(backend, w.expert_source, n_cache_slots, n_alt_slots)) {
        std::fprintf(stderr, "[moe] ExpertCache init failed\n");
        return 1;
    }
    ...
    if (!create_moe_state(backend, hidden, w.n_experts_active, max_verify, moe)) {
        std::fprintf(stderr, "[moe] MoeState init failed\n");
        return 1;
    }
